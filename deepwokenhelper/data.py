...
class DeepwokenData():
    def __init__(self, helper, buildId: str):
        ...
        shrine = {category: {sub_category: 0 for sub_category in attributes} for category, attributes in self.attributes.items()}
        ...
        for req_type, req_value in talent["reqs"].items():
            if isinstance(req_value, dict):
                for req, value in req_value.items():
                    if type == "normal":
                        body_value = max(self.data.attributes["base"]["Strength"], self.data.attributes["base"]["Fortitude"], self.data.attributes["base"]["Agility"])
                        mind_value = max(self.data.attributes["base"]["Intelligence"], self.data.attributes["base"]["Willpower"], self.data.attributes["base"]["Charisma"])
                        
                        if (req == "Body" and body_value < value):
                            return False
                        elif (req == "Mind" and mind_value < value):
                            return False
                        elif (req != "Body" and req != "Mind" and self.data.attributes[req_type].get(req, 0) < value):
                            return False
                        ...
                        body_value = max(
                            self.data.preShrine["base"]["Strength"] + self.data.racialStats["Strength"],
                            self.data.preShrine["base"]["Agility"] + self.data.racialStats["Agility"],
                            self.data.preShrine["base"]["Fortitude"] + self.data.racialStats["Fortitude"]
                            )
                        mind_value = max(
                            self.data.preShrine["base"]["Intelligence"] + self.data.racialStats["Intelligence"],
                            self.data.preShrine["base"]["Willpower"] + self.data.racialStats["Willpower"],
                            self.data.preShrine["base"]["Charisma"] + self.data.racialStats["Charisma"]
                            )
                        ...
                        if (req == "Body" and body_value < value):
                            ...
                            return False
                        elif (req != "Body" and req != "Mind" and self.data.preShrine[req_type].get(req, 0) + racial < value):
                            return False
                        ...
                        body_value = max(
                            self.data.postShrine["base"]["Strength"] + self.data.racialStats["Strength"],
                            self.data.postShrine["base"]["Agility"] + self.data.racialStats["Agility"],
                            self.data.postShrine["base"]["Fortitude"] + self.data.racialStats["Fortitude"]
                            )
                        mind_value = max(
                            self.data.postShrine["base"]["Intelligence"] + self.data.racialStats["Intelligence"],
                            self.data.postShrine["base"]["Willpower"] + self.data.racialStats["Willpower"],
                            self.data.postShrine["base"]["Charisma"] + self.data.racialStats["Charisma"]
                            )
                        ...
                        if (req == "Body" and body_value < value):
                            ...
                            return False
                        elif (req != "Body" and req != "Mind" and self.data.postShrine[req_type].get(req, 0) + racial < value):
                            return False
